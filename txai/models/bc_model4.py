...
from txai.models.encoders.simple import CNN, LSTM, GRU, Inception, TCN
...
class TimeXModel(nn.Module):
    ...
    def __init__(self,
        ...
        if self.ablation_parameters.archtype == 'transformer':
            ...
            self.d_z = 200
        elif self.ablation_parameters.archtype == 'inception':
            self.encoder_main = Inception(seq_len=max_len, 
                            num_classes=self.n_classes, 
                            in_channels=d_inp)
            self.d_z = 128

        elif self.ablation_parameters.archtype == 'tcn':
            if data_name in ['GunPoint', 'TwoPatterns']:
                model_params = {'channels':256, 'dilation_base':4, 'dropout': 0., 'n_blocks':4}
                self.encoder_main = TCN(
                            num_classes=self.n_classes, 
                            in_channels=d_inp,
                            **model_params)
                self.d_z = 256
        ...
        if self.ablation_parameters.archtype == 'transformer':
            ...
            self.encoder_t = Inception(seq_len=max_len, 
                            num_classes=self.n_classes, 
                            in_channels=d_inp)
        elif self.ablation_parameters.archtype == 'tcn':
            if data_name in ['GunPoint', 'TwoPatterns']:
