...
class MITECG(DataModule):
    ...
    def prepare_data(self):
        if not os.path.exists(os.path.join(self.data_dir,"split={}.pt".format(self.fold + 1))):
    ...
    def preprocess(self, split: str = "train") -> dict:
        if self.data == None:
            self.data = th.load(os.path.join(self.data_dir,"split={}.pt".format(self.fold + 1)), weights_only=False)
