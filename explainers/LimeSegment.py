...
def explain(
    ...
    for batch in tqdm(test_dataset):
        ...
        coef, index = LIMESegment(x.cpu().numpy().astype(np.float64), classifier, model_type='proba')
        attr_vector = torch.zeros_like(x)  # (T,)
        for seg_id, (start, end) in enumerate(zip(index[:-1], index[1:])):
            if end == -1:
                end = attr_vector.shape[0]
            attr_vector[start:end] = coef[seg_id]
        _attr.append(attr_vector.unsqueeze(-1))

    return torch.stack(_attr, axis=0)
...
def LIMESegment(example, model, model_type='class', distance='dtw', n=100, window_size=None, cp=None, f=None):
    ...
    torch_generated_samples_raw = torch.FloatTensor(generated_samples_raw).to(model.device).unsqueeze(-1)
    sample_predictions = model.predict(torch_generated_samples_raw).detach().cpu().numpy()
